...
CHANGELOG
  ...
  2026-05-16b Two-name structure. Metadata now carries cmc_name (CMC's
              internal label, e.g. "240BR" — traceability only, never
              shown) and display_name (= "Make Model", the customer-facing
              manufacturer proper name, e.g. "240 Sundeck"). The baked
              caption uses the customer-facing name. New optional
              --cmc-name CLI arg; defaults to --model when omitted.
"""
...
class Metadata:
    make: str
    model: str               # the CUSTOMER-FACING / manufacturer proper
                             #   model name (e.g. "240 Sundeck"). This is
                             #   what the builder shows the customer.
    cmc_name: str            # what CMC called this pattern internally
                             #   (e.g. "240BR", "H210"). Traceability only —
                             #   lets you map a pattern back to its CMC
                             #   source. NOT shown to the customer.
    display_name: str        # convenience: the string the builder should
                             #   display. = "{make} {model}". If model is
                             #   not yet corrected, a caller may set this to
                             #   the cmc_name as an honest placeholder.
    slug: str
    year_range: str         # e.g. "2003-2007"
    cmc_filename: str       # original source screenshot filename
...
def standardize(src_path: Path, make: str, model: str, year_range: str,
    ...
    # 5. Bake the CUSTOMER-FACING caption ("{make} {model}"). The customer
    #    sees the manufacturer proper name, never CMC's internal label.
    display_name = f"{make} {model}".strip()
    final = add_caption(snapped, display_name)
    ...
    meta = Metadata(
        make=make,
        model=model,
        cmc_name=cmc_name or model,   # if no distinct CMC label given,
                                      #   record the model itself
...
def main():
    ...
    ap.add_argument("--model", required=True,
                    help="CUSTOMER-FACING manufacturer proper name "
                         '(e.g. "240 Sundeck") — this is what the builder '
                         "shows and what gets baked into the caption")
    ap.add_argument("--cmc-name", default="",
                    help='CMC internal label if different (e.g. "240BR"). '
                         "Recorded for traceability, never shown to the "
                         "customer. Defaults to --model if omitted.")
